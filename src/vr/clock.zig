...
pub const Clock = ClockImpl(Time);
...
const Epoch = struct {
    ...
    fn elapsed(self: *Self, clock_monotonic_time: u64) u64 {
        return clock_monotonic_time - self.monotonic;
    }

    fn reset(self: *Self, replica: u8, clock_monotonic_time: u64, clock_realtime: i64) void {
        ...
        self.sources[replica] = Sample{
            .clock_offset = 0,
            .one_way_delay = 0,
        };
        self.samples = 1;
        self.monotonic = clock_monotonic_time;
...
pub fn ClockImpl(comptime ClockType: type) type {
    return struct {
        const Self = @This();
        ...
        pub fn init(
            ...
            self.epoch.reset(self.replica, self.monotonic(), self.realtime());
            // and open a new epoch window to start collecting samples...
            self.window.reset(self.replica, self.monotonic(), self.realtime());
        ...
        pub fn realtime_synchronized(self: *Self) ?i64 {
            if (self.synchronization_disabled) {
                ...
                const elapsed = @intCast(i64, self.epoch.elapsed(self.monotonic()));
        ...
        pub fn tick(self: *Self) void {
            ...
            if (self.epoch.elapsed(self.monotonic()) >= epoch_max) {
        ...
        fn synchronize(self: *Self) void {
            ...
            const elapsed = self.window.elapsed(self.monotonic());
            ...
            new_window.reset(self.replica, self.monotonic(), self.realtime());
...
const DeterministicClock = ClockImpl(DeterministicTime);
...
const ClockUnitTestContainer = struct {
    ...
    pub fn init(
        ...
        var time = try allocator.create(DeterministicTime);
        time.* = DeterministicTime{
    ...
    pub fn ticks_to_perform_assertions(self: *Self) [3]AssertionPoint {
        ...
        switch (self.time.offset_type) {
            .linear => {
                ...
                var threshold = self.owd / @intCast(u64, self.time.offset_coefficient_A);
                ret[0] = .{
                    .tick = threshold,
            ...
            .periodic => {
                ret[0] = .{
                    .tick = self.time.offset_coefficient_B / 4,
                    .expected_offset = @intCast(i64, self.owd),
                };
                ret[1] = .{
                    .tick = self.time.offset_coefficient_B / 2,
                    .expected_offset = 0,
                };
                ret[2] = .{
                    .tick = self.time.offset_coefficient_B * 3 / 4,
                    .expected_offset = -@intCast(i64, self.owd),
                };
            },
            .step => {
                ret[0] = .{
                    .tick = self.time.offset_coefficient_B - 10,
                    .expected_offset = 0,
                };
                ret[1] = .{
                    .tick = self.time.offset_coefficient_B + 10,
...
const ClockSimulator = struct {
    ...
    fn create_clock(self: *ClockSimulator, replica: u8) !DeterministicClock {
        // TODO: introduce stochastic clock with randomised variables
        var time = try self.allocator.create(DeterministicTime);
        time.* = DeterministicTime{
