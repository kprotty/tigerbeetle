...
pub fn SortedSegmentedArray(
    ...
    comptime element_count_max: u32,
    comptime Key: type,
    comptime key_from_value: *const fn (*const T) Key,
...
fn SegmentedArrayType(
    ...
    comptime Key: ?type,
    comptime key_from_value: if (Key) |K| (*const fn (*const T) K) else void,
    comptime compare_keys: if (Key) |K| (*const fn (K, K) math.Order) else void,
    comptime options: Options,
) type {
    return struct {
        const Self = @This();
        ...
        fn copy_backwards(
            ...
            const target_a = a[std.math.min(target, a.len)..std.math.min(target + source.len, a.len)];
        ...
        fn cursor_for_absolute_index(array: Self, absolute_index: u32) Cursor {
            ...
            const result = binary_search_keys(u32, struct {
                fn compare(a: u32, b: u32) math.Order {
...
fn TestContext(
    ...
    return struct {
        ...
        fn insert(context: *Self) !void {
            ...
            const count_max = std.math.min(count_free, TestArray.node_capacity * 3);
        ...
        fn remove(context: *Self) !void {
            ...
            const count_max = std.math.min(reference_len, TestArray.node_capacity * 3);
            const count = context.random.uintAtMostBiased(u32, count_max - 1) + 1;
