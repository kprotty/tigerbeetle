...
pub const IO = struct {
    ...
    pub fn init(entries: u12, flags: u32) !IO {
        ...
        assert(iocp != windows.INVALID_HANDLE_VALUE);
        return IO{ .iocp = iocp };
    }
    ...
    fn flush(self: *IO, wait_for_completions: bool) !void {
        ...
        // Only poll on IOCP if theres nothing currently completed & there's pending I/O.
        if (self.completed.peek() == null and self.io_inflight > 0) {
            // Default to poll without waiting (non-blocking)
            var poll_timeout_ms: windows.DWORD = 0;

            if (wait_for_completions) {
                const timeout_ns = next_timeout orelse @panic("IOCP blocking forever");
                const timeout_ms = @divCeil(timeout_ns, std.time.ns_per_ms);
                poll_timeout_ms = std.math.cast(windows.DWORD, timeout_ms) catch blk: {
                    break :blk std.math.maxInt(windows.DWORD);
                };

                // Make sure we're not blocking forver
                if (poll_timeout_ms == os.windows.INFINITE) {
                    poll_timeout_ms -= 1;
                }
            }

            var new_entries: windows.ULONG = 0;
            var entries: [256]windows.OVERLAPPED_ENTRY = undefined;
            const found_entries = windows.kernel32.GetQueuedCompletionStatusEx(
                self.iocp,
                &entries,
                @intCast(windows.ULONG, entries.len),
                &new_entries,
                poll_timeout_ms,
                windows.FALSE, // don't wait in an alertable state 
            );

            if (found_entries != windows.TRUE) {
            ...
            for (entries[0..new_entries]) |entry| {
                @compileError("TODO: WSAGetOverlappedResult() -> completion queue ");
            }
        }
